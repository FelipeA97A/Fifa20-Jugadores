...
public class BDController {
	...
	public BDController() {
		try {
			...
			//this.miConexion = DriverManager.getConnection("jdbc:mysql://localhost/fifa", "1GS", "Nelson2000");
			this.miConexion = DriverManager.getConnection("jdbc:mysql://localhost:3306/fifa", "root", "");
	...
	public ArrayList<Carta> dameCartas() {
		ArrayList<Carta> cartas = new ArrayList<Carta>();
		try {
			Statement st = this.miConexion.createStatement();
			ResultSet rs = st.executeQuery("SELECT * FROM cartas");
			while (rs.next()) {
				cartas.add(new Carta(rs.getString("nombre"), rs.getInt("cod_jugador"), 
						rs.getInt("rat"), rs.getString("pos"), rs.getInt("precio"), 
						rs.getInt("pac"), rs.getInt("sho"), rs.getInt("pas"), 
						rs.getInt("dri"), rs.getInt("def"), rs.getInt("phy"), 
						rs.getInt("pierna_mala"), rs.getInt("filigranas")));
			}
			rs.close();
			st.close();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			System.out.println("Fallo en el método dameCartas");
		}
		return cartas;
	}
		...
		try {
			...
			if (rs.first() == true) {
				carta = new Carta(rs.getString("nombre"), rs.getInt("cod_jugador"), rs.getInt("rat"), 
						rs.getString("pos"), rs.getInt("precio"), rs.getInt("pac"), rs.getInt("sho"), 
						rs.getInt("pas"), rs.getInt("dri"), rs.getInt("def"), rs.getInt("phy"), 
						rs.getInt("pierna_mala"), rs.getInt("filigranas"));
			}
			miStatement.close();
			rs.close();
